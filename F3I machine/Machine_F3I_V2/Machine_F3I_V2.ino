...
unsigned long startChrono1 = 0;       // temps de dÃ©marage chrono 1
unsigned long startChrono2 = 0;       // temps de dÃ©marage chrono 2
unsigned long tmpPasseChrono1 = 0;    // temps passÃ© depuis le lancement de chrono 1
...
void loop()
{
  ...
  if (StartVitesseC1 == HIGH && StateA1 == LOW && PrevStateA1 == HIGH && PassageBC1 == HIGH && LAP1 < 4 && Larguer == HIGH) // le concurent franchi Base A chrono 1 apres avoir passer B1
  {
    ...
    if (PassageAC2 == LOW)        // Si il y a pas dÃ©jÃ  eu un coup de klaxon pour AC2)
    {
      send(0x0002);              // passer
    }
  }
  if(StartVitesseC1 == HIGH && StateB1 == LOW && PrevStateB1 == HIGH && PassageAC1 == HIGH && LAP1 < 4 && Larguer == HIGH) // le concurent franchi Base B chrono 1 apres avoir passer A1
  {
    ++LAP1;                     // Ajoute un tour sur chrono 1
    PrevStateA1 = StateA1;      // Enregistre l'Ã©tat de A1 
    PrevStateB1 = StateB1;      // Enregistre l'Ã©tat de B1 
    PassageAC1 = LOW;           // prochaine appuit attendu en A chrono 1
    PassageBC1 = HIGH;          // Bloc l'appuit sur B chrono 1
    if (PassageBC2 == LOW)      // Si il y a pas dÃ©jÃ  eu un coup de klaxon pour AC2
    {
      send(0x0002);             // passer
    }
  }
  ...
  if (affichage == HIGH)
  {
    ...
    {
      tmpPasseChrono1 =   millis() - startChrono1;   // temps passÃ© depuis le lancement de chrono 1
    ...
    {
      tmpPasseChrono2 =   millis() - startChrono2;   // temps passÃ© depuis le lancement de chrono 2
